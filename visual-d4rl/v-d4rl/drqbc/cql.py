...
class CQLAgent:
    def __init__(
        ...
        self.critic = Critic(self.encoder.repr_dim,
                             action_shape, feature_dim, hidden_dim).to(device)
        self.critic_target = Critic(
            self.encoder.repr_dim, action_shape, feature_dim, hidden_dim).to(device)
    ...
    def update_critic(self, obs, action, reward, discount, next_obs, step):
        ...
        if self.offline:
            ...
            with torch.no_grad():
                ...
                curr_log_pis = curr_dist.log_prob(
                    curr_actions_tensor).sum(dim=-1, keepdim=True)

                new_curr_dist = self.actor(next_obs, stddev)
                new_curr_actions_tensor = new_curr_dist.sample(
                    clip=self.stddev_clip)
                new_log_pis = new_curr_dist.log_prob(
                    new_curr_actions_tensor).sum(dim=-1, keepdim=True)

            q1_rand, q2_rand = self.critic(obs, random_actions_tensor)
            q1_curr_actions, q2_curr_actions = self.critic(
            ...
            if self.cql_importance_sample:
                ...
                cat_q1 = torch.cat(
                    [q1_rand - random_density, q1_next_actions -
                        new_log_pis, q1_curr_actions - curr_log_pis], 1
                )
                cat_q2 = torch.cat(
                    [q2_rand - random_density, q2_next_actions -
                        new_log_pis, q2_curr_actions - curr_log_pis], 1
                )
            else:
                cat_q1 = torch.cat([q1_rand, Q1.unsqueeze(
                    1), q1_next_actions, q1_curr_actions], 1)
                cat_q2 = torch.cat([q2_rand, Q2.unsqueeze(
                    1), q2_next_actions, q2_curr_actions], 1)
