...
class CQLTrainer(TorchTrainer):
    ...
    def train_from_torch(self, batch):
        ...
        if self._num_train_steps < 1e5:
            ...
            if self.save_eigen and (self._num_train_steps % 1e4 == 0):
                flat_inputs = torch.cat((obs, actions), dim=1)
                eigen_num = self.qf1.get_eigen(flat_inputs)
                self.eval_statistics['effective_feature'] = eigen_num
            else:
                self.eval_statistics['effective_feature'] = 0.
            # if self.save_eigen:
            #     flat_inputs = torch.cat((obs, actions), dim=1)
            #     eigen_num = self.qf1.get_eigen(flat_inputs)
            #     # corr = torch.einsum('bi,bj->ij', [obs, obs])
            #     # eigval, _ = torch.eig(corr)
            #     # self.eval_statistics['effective_feature'] = ptu.get_numpy(torch.sum(eigval[:,0] > 0.001))
            #     self.eval_statistics['effective_feature'] = eigen_num
